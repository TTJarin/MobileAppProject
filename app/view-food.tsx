...
export default function ViewFoodScreen() {
  ...
  const currentUsername = currentUserEmail.split('@')[0]; // 🔥 adjust if you use UID/email
    ...
    const unsubscribe = onSnapshot(collection(db, 'foods'), async (snapshot) => {
      ...
      await Promise.all(
        usernames.map(async (username) => {
          ...
          if (userDoc.exists()) {
            namesMap[username] = userDoc.data().name || username;
          } else {
      ...
      {uniqueUsernames.length === 0 ? (
        ...
        uniqueUsernames.map((username) => (
          <TouchableOpacity
            key={username}
            style={styles.card}
            onPress={() => router.push({pathname:'/user-foods/[userId]',params:{userId:username}})} // ✅ Expo Router Navigation
          >
